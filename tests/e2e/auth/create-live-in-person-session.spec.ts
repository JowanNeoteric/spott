...
test.describe('Create and Remove Session', () => {
  ...
  test.describe('Live in Person:', () => {
    test('Single client - Basic activitiy', async () => {
      ...
      const generatedTitle = await createSession.generateSessionTitle(common.text.title.inPerson);
      ...
      await landing.verifySessionDetails(common.text.sessionDetails.type.inPerson, `${common.text.sessionDetails.label.singleParticipant} ${selectedClient}`, `${common.text.sessionDetails.label.joined} ${0}`, `${common.text.sessionDetails.label.activityType} ${common.text.activities.basic}`);
    ...
    test('Multi clients - Basic activity', async () => {
      ...
      await landing.verifyConfirmationPopup(common.text.confirmationPopup.success);
      await landing.findSessionInCalendar(generatedTitle);
      await landing.verifySessionDetails(common.text.sessionDetails.type.inPerson, `${common.text.sessionDetails.label.multiParticipants} ${participants}`, `${common.text.sessionDetails.label.joined} ${0}`, `${common.text.sessionDetails.label.activityType} ${common.text.activities.basic}`);
      await landing.removeSession();
      await landing.verifyConfirmationPopup(common.text.confirmationPopup.removed);
      await landing.isSessionRemoved();
    });
